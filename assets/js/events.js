...
function drawShowMoreButton(selector_id) {
  ...
  try {
    if (lines.length > limit) {
      ...
      let showMore =
        '<div class="col-6 col-12 align-center"><button onClick="toggleItems(toggle_' +
        selector_id +
        ');" class="button" id="toggle_' +
        selector_id +
        '">Show More</button></div>';
      selectorHTML.innerHTML += showMore;

      function isHidden(item, index) {
        // if the nb of items exceeds 4, hides the remainder
        if (index > limit - 1) {
...
function toggleItems(id) {
  switch (id.innerText) {
    ...
    default:
      console.error(error);
      console.log("button non-existant");
  }
}
